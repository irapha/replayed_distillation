...
def create_model(inputs, output_size):
    ...
    with tf.variable_scope('lenet-5_half'):
        ...
        with tf.variable_scope('conv1'):
            # Layer 1: Convolutional. Input = 32x32x1. Output = 28x28x3.
            conv1_w = tf.Variable(tf.truncated_normal([5,5,1,3], stddev=0.1), name='conv1_w')
            ...
            # MAKE SURE INPUTS ARE 32x32 IMAGES, otherwise the layer size below is wrong!!!
            layer_activations.append((conv1, 28*28*3))
        ...
        with tf.variable_scope('pool1'):
            # Pooling. Input = 28x28x3. Output = 14x14x3.
            pool_1 = tf.nn.max_pool(conv1, ksize=[1,2,2,1], strides=[1,2,2,1], padding='VALID')

        with tf.variable_scope('conv2'):
            # Layer 2: Convolutional. Output = 10x10x8.
            conv2_w = tf.Variable(tf.truncated_normal(shape=[5,5,3,8], stddev=0.1), name='conv2_w')
            conv2_b = tf.Variable(tf.zeros(8), name='conv2_b')
            conv2 = tf.nn.conv2d(pool_1, conv2_w, strides=[1,1,1,1], padding='VALID') + conv2_b

            layer_activations.append((conv2, 10*10*8))
        ...
        with tf.variable_scope('pool2'):
            # Pooling. Input = 10x10x8. Output = 5x5x8.
        ...
        with tf.variable_scope('fc1'):
            ...
            fc1_w = tf.Variable(tf.truncated_normal(shape=(200,120), stddev=0.1), name='fc1_w')
