...
def create_model(inputs, output_size):
    ...
    with tf.variable_scope('alex'):
        ...
        with tf.variable_scope('conv_pool_1'):
            conv1_1 = convLayer(inputs_reshaped, 11, 11, 4, 4, 32, "conv1_1", layer_activations, 56*56*32, init_dict=imnet)
            pool1 = maxPoolLayer(conv1_1, 3, 3, 2, 2, "pool1")

        with tf.variable_scope('conv_pool_2'):
            conv2_1 = convLayer(pool1, 5, 5, 1, 1, 96, "conv2_1", layer_activations, 28*28*96, init_dict=imnet)
            pool2 = maxPoolLayer(conv2_1, 3, 3, 2, 2, "pool2")

        with tf.variable_scope('conv_pool_3'):
            conv3_1 = convLayer(pool2, 3, 3, 1, 1, 192, "conv3_1", layer_activations, 14*14*192, init_dict=imnet)

        with tf.variable_scope('conv_pool_4'):
            conv4_1 = convLayer(conv3_1, 3, 3, 1, 1, 192, "conv4_1", layer_activations, 14*14*192, init_dict=imnet)

        with tf.variable_scope('conv_pool_5'):
            conv5_1 = convLayer(conv4_1, 3, 3, 1, 1, 128, "conv5_1", layer_activations, 14*14*128, init_dict=imnet)
            pool5 = maxPoolLayer(conv5_1, 3, 3, 2, 2, "pool5")

        with tf.variable_scope('fc_6'):
            fcIn = tf.reshape(pool5, [-1, 7*7*128])
            fc6 = fcLayer(fcIn, 7*7*128, 2048, True, "fc6", layer_activations, init_dict=imnet)
            dropout1 = dropout(fc6, keep_prob)

        with tf.variable_scope('fc_7'):
            fc7 = fcLayer(dropout1, 2048, 2048, True, "fc7", layer_activations, init_dict=imnet)
        ...
        with tf.variable_scope('fc_8'):
            ...
            with tf.variable_scope("fc8") as scope:
                w = tf.Variable(
                        tf.truncated_normal(shape=[2048, output_size], stddev=np.sqrt(2.0/4096)),
                        name='fc8_w')
                        #  tf.truncated_normal(shape=[2048, output_size], stddev=0.01),
                        #  name='fc8_w')
...
def load_model(sess, model_meta, model_checkpoint, output_size):
    ...
    layer_activations = []
    #  layer_activations.append((tf.get_collection('conv1_1')[0], 56*56*32))
    #  layer_activations.append((tf.get_collection('conv2_1')[0], 28*28*96))
    #  layer_activations.append((tf.get_collection('conv3_1')[0], 14*14*192))
    #  layer_activations.append((tf.get_collection('conv4_1')[0], 14*14*192))
    #  layer_activations.append((tf.get_collection('conv5_1')[0], 14*14*128))
    #  layer_activations.append((tf.get_collection('fc6')[0], 2048))
    #  layer_activations.append((tf.get_collection('fc7')[0], 2048))
...
def load_and_freeze_model(sess, inputs, model_meta, model_checkpoint, batch_size, output_size):
    ...
    with tf.variable_scope('alex'):
        ...
        with tf.variable_scope('conv_pool_1'):
            ...
            #  layer_activations.append((conv1_1, 56*56*32))
        ...
        with tf.variable_scope('conv_pool_2'):
            ...
            #  layer_activations.append((conv2_1, 28*28*96))
        ...
        with tf.variable_scope('conv_pool_3'):
            ...
            #  layer_activations.append((conv3_1, 14*14*192))
        ...
        with tf.variable_scope('conv_pool_4'):
            ...
            #  layer_activations.append((conv4_1, 14*14*192))
        ...
        with tf.variable_scope('conv_pool_5'):
            ...
            #  layer_activations.append((conv5_1, 14*14*128))
        ...
        with tf.variable_scope('fc_6'):
            ...
            #  layer_activations.append((fc6, 2048))
        ...
        with tf.variable_scope('fc_7'):
            ...
            #  layer_activations.append((fc7, 2048))
