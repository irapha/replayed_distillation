# symmetric bilinear cnn, vgg19 as conv stack
#
...
def create_model(inputs, output_size):
    ...
    with tf.variable_scope('vgg'):
        ...
        inputs_reshaped = tf.reshape(inputs, [-1, 448, 448, 1])

        with tf.variable_scope('conv_pool_1'):
            conv1_1 = convLayer(inputs_reshaped, 3, 3, 1, 1, 64, "conv1_1", layer_activations, 448*448*64)
            conv1_2 = convLayer(conv1_1, 3, 3, 1, 1, 64, "conv1_2", layer_activations, 448*448*64)
            pool1 = maxPoolLayer(conv1_2, 2, 2, 2, 2, "pool1")

        with tf.variable_scope('conv_pool_2'):
            conv2_1 = convLayer(pool1, 3, 3, 1, 1, 128, "conv2_1", layer_activations, 224*224*128)
            conv2_2 = convLayer(conv2_1, 3, 3, 1, 1, 128, "conv2_2", layer_activations, 224*224*128)
            pool2 = maxPoolLayer(conv2_2, 2, 2, 2, 2, "pool2")

        with tf.variable_scope('conv_pool_3'):
            conv3_1 = convLayer(pool2, 3, 3, 1, 1, 256, "conv3_1", layer_activations, 112*112*256)
            conv3_2 = convLayer(conv3_1, 3, 3, 1, 1, 256, "conv3_2", layer_activations, 112*112*256)
            conv3_3 = convLayer(conv3_2, 3, 3, 1, 1, 256, "conv3_3", layer_activations, 112*112*256)
            conv3_4 = convLayer(conv3_3, 3, 3, 1, 1, 256, "conv3_4", layer_activations, 112*112*256)
        ...
        """ 
        with tf.variable_scope('bilinear_interpolation'):
            outer_product = tf.einsum('ijkm,ijkn->imn', pool3, pool3)#conv5_4, conv5_4)
            outer_product = tf.reshape(outer_product,[-1, 256*256])
            mean_product = tf.divide(outer_product,784.0)  
            bilinear = tf.multiply(tf.sign(mean_product),tf.sqrt(tf.abs(mean_product)+1e-12))
            normalized_bilinear = tf.nn.l2_normalize(bilinear, dim=1)

        with tf.variable_scope('fc_6'):
            fcIn = tf.reshape(normalized_bilinear, [-1, 256*256])
            fc6 = fcLayer(fcIn, 256* 256, 4096, True, "fc6", layer_activations)
